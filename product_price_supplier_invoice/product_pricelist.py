    ...
    def _price_rule_get_multi(self, cr, uid, pricelist, products_by_qty_by_partner, context=None):
        ...
        for product, qty, partner in products_by_qty_by_partner:
            ...
            for rule in items:
                ...
                elif rule.base == -9:   # Calculate the price based on the latest supplier invoice price
                    cr.execute("SELECT il.price_unit, i.currency_id, uos_id "
                               " FROM account_invoice_line il JOIN account_invoice i ON i.id=il.invoice_id"
                               " WHERE i.type='in_invoice' AND i.state NOT IN ('cancel', 'draft') "
                               "   AND product_id=%s"
                               " ORDER BY i.number DESC LIMIT 1", (product.id,))
                    for price_unit, currency_id, uos_id in cr.fetchall():
                        price_uom_id = uos_id
                        price = currency_obj.compute(cr, uid, currency_id, pricelist.currency_id.id, price_unit, round=False, context=context)
                        break
                    if not price and rule.base_pricelist_id:    # Fallback to rule.base == -1
                        if rule.base_pricelist_id:
                            price_tmp = self._price_get_multi(cr, uid,
...
class ProductPriceListItem(orm.Model):
    ...
    def _price_field_get(self, cr, uid, context=None):
        result = super(ProductPriceListItem, self)._price_field_get(cr, uid, context=context)
        result.append((-9, _('Latest supplier invoice price')))
        return result
